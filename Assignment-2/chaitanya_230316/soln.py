...
def calculate_letter_frequencies(text):
    letter_count = Counter(c for c in text if chr(c) in string.ascii_letters)
...
def filter_printable(text):
    return ''.join(chr(c) for c in text if chr(c) in string.printable)
...
decrypted_filtered = filter_printable(decrypted)

print("Decrypted text:", decrypted_filtered)
